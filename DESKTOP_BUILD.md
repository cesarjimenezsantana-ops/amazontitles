...
The installer is created in `dist/FocusAmazonTools-1.1.8-macOS-<architecture>.dmg`. The version defaults to `version.py`; set `APP_VERSION` only to override it.
...
- `dist\FocusAmazonTools-Setup-1.1.8.exe` — per-user installer.
- `dist\FocusAmazonTools-Portable-1.1.8-Windows-x64.exe` — portable executable that runs without installation.
...
Run it manually with a version number or push a tag such as `v1.1.8`. Each installer is uploaded as a workflow artifact.
