...
```bash
APP_VERSION=1.1.1 ./scripts/build_macos.sh
```

The installer is created in `dist/FocusAmazonTools-1.1.1-macOS-<architecture>.dmg`.
...
```powershell
.\scripts\build_windows.ps1 -AppVersion 1.1.1
```

The build creates both:

- `dist\FocusAmazonTools-Setup-1.1.1.exe` — per-user installer.
- `dist\FocusAmazonTools-Portable-1.1.1-Windows-x64.exe` — portable executable that runs without installation.
...
Run it manually with a version number or push a tag such as `v1.1.1`. Each installer is uploaded as a workflow artifact.
